...
def generate_qr():
    ...
    # ปรับขนาด QR Code ตามความละเอียดที่เลือก
    if resolution == "Low":
        resize_size = (150, 150)
        ...
        resize_size = (300, 300)  # ค่าเริ่มต้น

    qr_img = qr_img.resize(resize_size, Image.Resampling.NEAREST)
...
def switch_type(*args):
    if qr_type.get() == "WiFi":
        wifi_frame.grid(row=3, column=0, columnspan=2, pady=10, sticky="ew")
        url_frame.grid_remove()
    else:
        wifi_frame.grid_remove()
        url_frame.grid(row=3, column=0, columnspan=2, pady=10, sticky="ew")
...
# ตัวเลือกความละเอียด
tk.Label(root, text="เลือกความละเอียด:").grid(row=1, column=0, padx=10, pady=10)
resolution_var = tk.StringVar(value="Medium")
resolution_menu = ttk.Combobox(root, textvariable=resolution_var, values=["Low", "Medium", "High"], state="readonly")
...
ttk.Button(root, text="สร้าง QR Code", command=generate_qr).grid(row=4, column=0, padx=10, pady=10)
ttk.Button(root, text="บันทึก QR Code", command=save_qr).grid(row=4, column=1, padx=10, pady=10)
